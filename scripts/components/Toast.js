...
window.Toast = ({ message, onClose, duration = 5000, title = "Notification", icon = null }) => {
  ...
  return (
    <div
      className={`fixed bottom-12 left-1/2 transform -translate-x-1/2 z-[10000] flex items-center justify-center rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.5)] min-w-[320px] overflow-hidden ${isAchievement ? 'p-[3px]' : 'bg-white dark:bg-gray-900 border border-indigo-100 dark:border-indigo-900'}`}
    ...
    >
      ...
      <div className={`relative z-10 flex items-center gap-4 w-full h-full ${isAchievement ? 'bg-white dark:bg-gray-900 rounded-xl pl-4 pr-6 py-3' : 'pl-4 pr-6 py-3'}`}>
        {/* Icon Circle */}
