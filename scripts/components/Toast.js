...
window.Toast = ({ onClose, message, title = "Notification", icon = null, duration = window.TOAST_DURATION }) => {
  ...
  return (
    <div className="fixed bottom-12 left-1/2 transform -translate-x-1/2 z-[10000]">
      ...
      <div
        className={`flex items-center justify-center rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.5)] min-w-[340px] overflow-hidden ${isSpecial
          ? "p-[3px]"
          : "bg-white dark:bg-gray-900 border border-indigo-100 dark:border-indigo-900"
          }`}
        style={{ animation: isError ? "shake 0.5s cubic-bezier(.36,.07,.19,.97) both" : "xboxPop 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards" }}
      >
        {isLevelUp && (
        ...
        <div
          className={`relative z-10 flex items-center gap-4 w-full h-full ${isSpecial ? "bg-white dark:bg-gray-900 rounded-xl pl-4 pr-6 py-3" : "pl-4 pr-6 py-3"
            }`}
        >
          {/* Icon Circle */}
          <div
            className={`flex-shrink-0 w-12 h-12 rounded-full flex items-center justify-center ${isAchievement
          ...
          >
            {icon ? (
              <span className={`text-2xl ${isError ? "text-red-600 dark:text-red-400" : ""}`}>{icon}</span>
            ) : isLevelUp ? (
              <TrendingUpIcon
                size={24}
          ...
          <div className="flex flex-col flex-grow">
            <span
              className={`text-xs font-bold tracking-wide uppercase ${isAchievement
