  ...
  return (
    ...
    >
      ...
      <div className={`relative z-10 flex items-center gap-4 w-full h-full ${isSpecial ? 'bg-white dark:bg-gray-900 rounded-xl pl-4 pr-6 py-3' : 'pl-4 pr-6 py-3'}`}>
        {/* Icon Circle */}
        <div className={`flex-shrink-0 w-12 h-12 rounded-full flex items-center justify-center ${isLevelUp ? 'bg-yellow-100 dark:bg-yellow-900/30' : 'bg-indigo-50 dark:bg-indigo-900/30'}`}>
          {icon ? <span className="text-2xl">{icon}</span> : (
            isLevelUp ?
              <TrendingUpIcon size={24} className="text-yellow-600 dark:text-yellow-400 animate-bounce" /> :
        ...
        <div className="flex flex-col flex-grow">
          <span className={`text-xs font-bold tracking-wide uppercase ${isLevelUp ? 'text-yellow-600 dark:text-yellow-400' : 'text-indigo-500 dark:text-indigo-400'}`}>
            {title}
          </span>
